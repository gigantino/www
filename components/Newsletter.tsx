...
const newBadge = (
  <div className="absolute -top-8 -right-8">
    <svg viewBox="-3 -3 106 106" className="w-26 h-26 animate-spin" style={{ animationDuration: "20s" }}>
      <path
    ...
    <span className="absolute inset-0 flex items-center justify-center text-lg font-extrabold rotate-[20deg]">
      NEW
    </span>
  </div>
);
...
export function Newsletter() {
  ...
  return (
    <BentoCard id="newsletter" className="relative flex flex-col gap-4 bg-pink-200">
      {newBadge}
      <h2 className="text-xl font-bold flex items-center gap-2"><Mail className="size-5" /> Newsletter</h2>
      <p className="text-base">{newsletter.description}</p>
      {message ? (
        ...
        <form onSubmit={handleSubmit} className="flex gap-2">
          <input
            ...
            required
            className="neo-card min-w-0 flex-1 bg-emerald-100 px-3 py-2 text-base outline-none"
          />
          <button
            type="submit"
            disabled={loading}
            className="neo-button shrink-0 bg-yellow-100 text-base disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1"
          >
