...
If you use [Vercel](https://vercel.com/) or [Netlify](https://netlify.com/), chances are you don't. Both offer ways to get Lighthouse reports automatically in their different ways. Vercel calls them "[Speed insights](https://vercel.com/docs/speed-insights/)". They are updated even after your first deployment and display historical data with graphs. Netlify, on the other hand, has a [Lighthouse extension](https://docs.netlify.com/monitor-sites/lighthouse/) that shows standard lighthouse reports on every build.
...
I'm genuinely impressed that [Unlighthouse](https://unlighthouse.dev/) isn't more popular. The project only has [~3000 stars on GitHub](https://github.com/harlan-zw/unlighthouse) at the time of writing, but in my opinion it absolutely deserves more. It offers a powerful CLI that, on average, can scan entire websites in _two minutes!_
...
> <span>Don't worry if you don't specifically use Microsoft Teams! Most services out there support webhooks, which means that you could potentially make it work with any platform by doing some minor changes.</span>

## GitHub webhook setup

With my current requirements, I needed Unlighthouse to run on each new build. Deploying the action on each commit (which is how we fire our development builds) isn't smart since builds take longer than Unlighthouse to deploy. This meant that I needed to run a webhook instead.

Unfortunately, GitHub actions don't support webhooks and use [workflow dispatches](https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows) instead. In short, to fire them, you are required to send a bearer token like so:
...
In my case, I’m using Netlify to fire webhooks, and all of their requests are signed using [JWS](https://docs.netlify.com/site-deploys/notifications/#payload-signature). Their docs have an example written in Ruby, and rewriting it in your language of choice shouldn’t be too difficult.
...
This script _can_ and _will_ run locally if you want to test it by running the following commands. Keep in mind that should add `.unlighthouse` to your `.gitignore`.
